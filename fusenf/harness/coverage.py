...
CORPORA = {
    "tierA":    ("tierA.jsonl",    ["tierA.parses.jsonl", "tierA_p1.parses.jsonl",
                                    "tierA_p2.parses.jsonl", "tierA_p3.parses.jsonl"], "substrate (batch 1)"),
    ...
    "tierC":    ("tierC.jsonl",    ["tierC_p1.parses.jsonl", "tierC_p2.parses.jsonl",
                                    ...
                                    "tierC.parses.jsonl", "reparse30.parses.jsonl"], "substrate"),
...
def load_operative_parses():
    ...
    best = {}
    for stores in {s for _, ss, _ in CORPORA.values() for s in ss}:
...
def main() -> None:
    ...
    lines = [f"# Coverage dashboard — current prompt `{cur[:8]}`",
             ...
             "policy); substrate-elig = total − defect − flagged-open − never-parsed.",
             "",
             ...
             "|---|---|---|---|---|---|---|---|---|---|---|---|"]
    for name, (roster, _stores, role) in CORPORA.items():
        ...
        elig = (str(n - states['defect-awaiting-reparse'] - states['flagged-open']
                    - states['never-parsed'])
