# Blind review brief — QA task stream (schema.md §5.2 — the parse reviewer)
...
Your batch file `batches/review/rv-NN.txt` has one item per line, tab-separated, in one of two
shapes:
...
absolute path of a file whose lines are the prior atoms the parser was given as the `CONTEXT:`
block of the instruction set's input form (*Input — TEXT and optional CONTEXT*); read it, it is
part of the item. A question was to be translated into the query line(s) the instruction set's
*Queries* section specifies (`(: $prf <pattern> $tv)`), an explanation question per
*Explanation questions* — judge a query by the same four questions as a parse.
...
1. **Faithful?** Is this parse right for this sentence — roles, negation, scope, tense,
   quantification, compound decomposition, truth values; for a query, the unknowns, the
   bound names, and (with context) the reuse of the context's symbols for the referents it
   identifies? Mechanically valid output that means the wrong thing is a defect. Judge against
   the instruction set as written, not against how you would have designed it.
...
3. **Context leak?** If the item supplied a context file, did the parse re-emit a context atom
   or assert context content as new fact? (A failure to reuse a context symbol is a question-1
   issue, not a leak.) If no context was supplied, answer `na`.
