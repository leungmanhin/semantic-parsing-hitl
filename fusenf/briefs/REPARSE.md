# Blind re-parse brief (run 2)

Identical to `PARSE.md` in every respect **except the output filename**: write to

    /home/manhin/Dev/semantic-parsing-hitl/fusenf/raw/<ID>__run2.txt

(not `__run1.txt` — those are earlier parses and must not be touched).
