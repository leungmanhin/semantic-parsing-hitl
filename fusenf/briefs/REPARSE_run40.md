# Blind re-parse brief (run 40 — batch-2 Tier C uniform-hash wave)

Identical to `PARSE.md` in every respect **except the output filename**: write to

    /home/manhin/Dev/semantic-parsing-hitl/fusenf/raw/<ID>__run40.txt

(not `__run1.txt` — earlier runs are earlier parses and must not be touched).
