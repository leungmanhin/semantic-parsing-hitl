# Blind adjudication brief — QA task stream (the review-gate adjudicator)
...
Your batch file `batches/adjudication/aj-NN.txt` has one item per line, tab-separated, in
one of two shapes:
...
absolute path of a file whose lines are the prior atoms the parser was given as the
`CONTEXT:` block of the instruction set's input form (*Input — TEXT and optional CONTEXT*);
read it, it is part of the item. A question was to be translated into the query line(s) the
instruction set's *Queries* section specifies (`(: $prf <pattern> $tv)`), an explanation
question per *Explanation questions*.

For each item, read exactly two files (three when a context file is named):
...
- `confirm` — the issue is real: the parse violates a rule the instruction set states,
  or it puts false, ill-typed, or over-asserted content into the knowledge base (for a
  query: asks something other than the question, or binds the wrong slot). Cite the rule
  or the falsehood in one sentence.
