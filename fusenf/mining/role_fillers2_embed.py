...
a slot, raw cluster distributions, JSD gate (owner 2026-09-05) and, on request, the role-pair level;
...
Role pairs: every head pair. Slot pairs: every pair clearing the >= 2 shared-cluster guard
...
JSONL holds them). Role-pair floor 14 (``--min-role``; owner 2026-09-07; 20 in the 2026-09-04/05 runs).

Usage:
  python role_fillers2_embed.py [--arm faithful] [--emb-dir out_h/embeddings] [--emb-mode word|subtree|both]
      [--cluster-cos 0.80,0.85,0.90,0.95,1.0] [--min-n 3] [--min-role 14] [--jsd-max 0.3] [--cos 0.5]
      [--metta-main word@0.85] [--levels slot|role|both]

``--levels`` (2026-09-15): ``slot`` (DEFAULT) = the paper's method, (class, head) slot pairs.
``role`` = OUR EXTENSION, role pairs = one head's fillers pooled over every class (the paper's
"Agent2" most likely names the second participant slot of the "go to" meta-node of §4.3.1, i.e. a
slot, not a pooled role). ``both`` = the two levels in one file. ``role`` alone writes its own
files: ``rolefillers2_additions_rolepairs[_<gate>].{md,metta}`` + ``_dial/`` (faithful arm;
``rolefillers2_augmented_rolepairs*.metta`` / ``rolefillers2_compare_rolepairs*.md`` for the
augmented arm) and the record ``rolefiller2_rolepairs_<arm>[_<gate>]_<mode>_<cut>.jsonl`` (every
head pair with its gate verdict). The slot-level signals JSONL and the Tier A scorecard exist only
when the slot level runs; the slotdist JSONL and the cluster files are written either way.
"""
...
def main():
    ...
    ap.add_argument("--arm", choices=("faithful", "augmented"), default="augmented",
                    help="faithful = paper §4.3.2 as written: EVERY event-center head is a slot, raw "
                         ...
                         "included), cosine + JSD (the role-pair level is our extension: --levels role|both); augmented = the "
                         "role/oblique slot set with the chosen weighting (our additions)")
    ap.add_argument("--min-role", "--min-pooled", dest="min_role", type=int, default=14,
                    help="min embedded fillers for a head to enter the role-pair level (our extension, --levels "
                         "role|both; owner 2026-09-07: 14, so every legislated role that reaches it is audited — "
                         "CoAgent 16 / Beneficiary 14 on H; the 2026-09-04/05 runs used 20; any floor 18..29 selects "
                         "the same 17 heads on H; --min-pooled = the name before 2026-09-15)")
    ap.add_argument("--levels", choices=("slot", "role", "both"), default="slot",
                    help="which pair levels to compute and render: 'slot' (default) = the paper's method, (class, head) "
                         "slots of one centre kind against each other; 'role' = OUR EXTENSION, role pairs = one head's "
                         "fillers pooled over every class, written to its own *_rolepairs* files; 'both' = the two "
                         "levels in one file. The slot-level record files (signals JSONL) and the Tier A scorecard "
                         "exist only when the slot level runs; the slotdist JSONL and the cluster files are written "
                         "either way")
    ...
    do_role, do_slot = args.levels != "slot", args.levels != "role"   # the pair levels this run renders
    if faithful:
        args.weighting = "raw"
    ...
    lv = "_rolepairs" if args.levels == "role" else ""   # role pairs alone = our extension -> its own files
    args.report = args.report or os.path.join(
        args.out_dir, (f"rolefillers2_additions_rolepairs{gtag}.md" if faithful and lv else
                       f"rolefillers2_faithful{gtag}.md" if faithful else f"rolefillers2_compare{lv}{gtag}.md"))
    sig_prefix = f"rolefiller2_signals_faithful{gtag}_" if faithful else f"rolefiller2_signals_embed{gtag}_"
    rp_prefix = f"rolefiller2_rolepairs_{args.arm}{gtag}_"   # the role-pair record (written when that level runs)
    if args.metta_out is None:
        args.metta_out = os.path.join(args.out_dir, (f"rolefillers2_additions_rolepairs{gtag}.metta"
                                                     if faithful and lv else f"rolefillers2_{args.arm}{lv}{gtag}.metta"))
    ...
    if faithful:
        R = ["# §4.3.2 Role-Filler Distribution Clustering — FAITHFUL arm"
             + (" — ROLE PAIRS (our extension, not the paper's method)" if args.levels == "role" else " (paper as written)") + "\n",
             ...
             "embedded fillers" + (f"; a head enters the role-pair level (our extension) at n >= {args.min_role}"
                                   if do_role else "") + " |",
    ...
    for m in modes:
        for thr in thresholds:
            ...
            role_mass = collections.defaultdict(collections.Counter)   # head -> cluster mass (event centres)
            slot_texts = collections.defaultdict(collections.Counter)   # slot -> its own filler texts (mass)
            role_texts = collections.defaultdict(collections.Counter)  # head -> filler texts pooled over every class
            ...
            for r, units in zip(occ, occ_units[m]):
                ...
                for text, share in units:
                    ...
                    if head_class(r["center_kind"], r["head"]) is not None:   # argument slots only
                        role_mass[r["head"]][u] += share
                        if not is_wild(u):
                            role_texts[r["head"]][text] += share
                    slots[key][u] += share
                    if not is_wild(u):
            ...
            else:   # --levels role: no slot pairs, no slot-level record files
                ev_same_role, ev_same_event, ev_cross_both, ev_raw = [], [], [], ()
                en_same_role, en_same_center, en_cross_both, en_raw = [], [], [], ()
            sens = None
            if do_slot and faithful and args.gate == "jsd" and args.jsd_sensitivity:
            ...
            # role-pair level (our extension; heads pooled over every class): every head pair
            role_pairs = []
            heads = sorted(h for h in role_mass if sum(role_mass[h].values()) >= args.min_role) if do_role else []
            for i, a in enumerate(heads):
                for b in heads[i + 1:]:
                    role_pairs.append({"a": a, "b": b, "n_a": mass(sum(role_mass[a].values())),
                                         "n_b": mass(sum(role_mass[b].values())),
                                         "cosine": round(cosine(role_mass[a], role_mass[b]), 3),
                                         "jsd": jsd(role_mass[a], role_mass[b]),
                                         "shared": [render_unit(u) for u in sorted(
                                             set(role_mass[a]) & set(role_mass[b]),
                                             key=lambda u: (-(role_mass[a][u] + role_mass[b][u]), u))[:3]]})
            role_pairs.sort(key=lambda r: (-r["cosine"], r["a"], r["b"]))
            if do_role:   # the role-pair record: every head pair with its gate verdict
                with open(os.path.join(args.out_dir, f"{rp_prefix}{tag}.jsonl"), "w", encoding="utf-8") as fh:
                    for r in role_pairs:
                        sh = sorted(u for u in set(role_mass[r["a"]]) & set(role_mass[r["b"]]) if not is_wild(u))
                        ok = len(sh) >= 2 and ((r["jsd"] is not None and r["jsd"] <= args.jsd_max)
                                               if args.gate == "jsd" else r["cosine"] >= args.cos)
                        fh.write(json.dumps({
                            "pair": {"head_a": r["a"], "head_b": r["b"]}, "n_a": r["n_a"], "n_b": r["n_b"],
                            "cosine": r["cosine"], "jsd": r["jsd"], "shared_clusters": sh,
                            "shared_rendered": [render_unit(u) for u in sh[:6]], "pass": ok, "gate": args.gate,
                            "kind": "role-merge", "min_role": args.min_role,
                            "method": f"role-fillers2-4.3.2-embed/{m}@{thr}", "variant": args.arm,
                            "additions": ["role-pair level: heads pooled over every class (our extension, not in the paper)"],
                        }, ensure_ascii=False, sort_keys=True) + "\n")
            ...
            results[(m, thr)] = {
                ...
                "role_dist": role_dist, "members": members, "role_mass": role_mass, "role_pairs": role_pairs,
    ...
    for m in modes:
        for thr in thresholds:
            ...
            role_dist, members, role_pairs = R_["role_dist"], R_["members"], R_["role_pairs"]
            ...
            if faithful:
                ...
                if do_role:
                    R.append("### role pairs — each head's fillers pooled over every class (OUR EXTENSION, not the paper's method)\n")
                    R.append("| cosine | JSD | head A (n) | head B (n) | shared clusters |\n|---|---|---|---|---|")
                    for r in role_pairs[:20]:
...
def write_metta(path, args, modes, thresholds, results, n_texts, dim, n_occ):
    ...
    block = the role pairs (our extension, --levels role|both; EVERY head pair, gate PASS / FAIL) and the gated slot
    ...
    ``--levels role|slot`` renders one level only (a header line says so)."""
    faithful = args.arm == "faithful"
    gate_jsd = args.gate == "jsd"
    do_role, do_slot = args.levels != "slot", args.levels != "role"
    ...
    def write_header(fh, m, thr, is_main, R_):
        title_lv = " — ROLE PAIRS (our extension)" if args.levels == "role" else ""
        gsfx = "" if gate_jsd == faithful else "_" + args.gate
        fh.write(f";; FUSE-NF §4.3.2 Role-Filler Distribution Clustering — {args.arm.upper()} arm{title_lv} — readable MeTTa RENDERING\n"
                 ";; Never loaded: a slot is a conjunctive query over variables, not an assertion. The record of truth is\n"
                 + (f";;   rolefiller2_signals_{'faithful' if faithful else 'embed'}{gsfx}_<mode>_<cut>.jsonl   (the gated slot pairs)\n"
                    if do_slot else "")
                 + (f";;   rolefiller2_rolepairs_{args.arm}{gsfx}_<mode>_<cut>.jsonl   (every role pair with its gate verdict)\n"
                    if do_role else "")
        ...
        if do_role:
            fh.write(f";;   role-pair floor a head enters the role-pair level at n >= {args.min_role} embedded fillers\n")
        fh.write({"slot": ";;   levels          slot pairs = the paper's method (--levels slot, the default); the role-pair level is\n"
                          ";;                   our extension, rendered on request (--levels role -> the *_rolepairs* files)\n",
                  "role": ";;   levels          role pairs only = OUR EXTENSION, not the paper's method (--levels role): one head's\n"
                          ";;                   fillers pooled over every class; the paper's slot pairs are the default run\n",
                  "both": ";;   levels          slot pairs (the paper's method) + role pairs (our extension) in one file (--levels both)\n"
                  }[args.levels])
        ...
        listing = ([";; Role pairs: every head pair."] if do_role else []) + ([
            ...
            ";; Slot pairs: every pair of the same centre kind (--metta-fail-slots all)."] if do_slot else [])
        fh.write("\n".join(listing) + "\n"
    ...
    def write_block(fh, m, thr, R_):
        role_mass, compared, members = R_["role_mass"], R_["compared"], R_["members"]
        slot_texts, role_texts = R_["slot_texts"], R_["role_texts"]
        ...
        if do_role:
            ...
            for r in R_["role_pairs"]:
                sh = {u for u in set(role_mass[r["a"]]) & set(role_mass[r["b"]]) if not is_wild(u)}
            ...
            fh.write(f"\n;; ==================== ROLE PAIRS: {len(heads)} heads at n >= {args.min_role}, "
                     f"{len(pp)} pairs, {n_pass} pass the gate ====================\n"
                     ";; (OUR EXTENSION, not the paper's method: one head's fillers pooled over every class, against every other\n"
                     ";; head's; the paper's 'Agent2' most likely names the second participant slot of the 'go to' meta-node, i.e. a slot)\n")
            for r in pp:
                fh.write(f"\n;; [role pair]  {r['a']} ~ {r['b']}   n {r['n_a']} / {r['n_b']}   gate: {'PASS' if r['ok'] else 'FAIL'}\n")
                fh.write(f";; A fillers: {top_fillers(role_texts[r['a']])}\n")
                fh.write(f";; B fillers: {top_fillers(role_texts[r['b']])}\n")
